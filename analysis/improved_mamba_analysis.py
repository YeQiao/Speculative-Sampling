#!/usr/bin/env python3
"""
Improved Mamba Training Progress Analysis

Focus: Analyzing the improved-mamba-alignment training checkpoints
Goal: Understand training progression and optimal checkpoint selection
Comparison: Advanced vs Improved training approaches
"""
...
def extract_checkpoint_number(model_name):
    ...
    if 'improved-250' in model_name:
        return 250
    elif 'improved-500' in model_name:
        return 500
    elif 'improved-750' in model_name:
        return 750
    elif 'improved-1000' in model_name:
        return 1000
    elif 'improved-1250' in model_name:
        return 1250
    elif 'improved-1500' in model_name:
        return 1500
    elif 'improved-best' in model_name:
        return 9999  # Special marker for best
    elif 'improved-final' in model_name:
...
def analyze_training_progression(results, output_dir, target_filter=None, tf32_filter=None):
    ...
    print("\n" + "="*120)
    print(f"TRAINING PROGRESSION ANALYSIS - IMPROVED MAMBA")
    ...
    # Filter for improved Mamba models only
    improved_exps = [e for e in spec_exps if 'improved' in e['draft_model'].lower()]
    
    if not improved_exps:
        print("No improved Mamba experiments found!")
        return
...
def compare_with_baseline_and_llama(results, baseline_results, output_dir, target_filter=None, tf32_filter=None):
    """
    Compare improved-mamba-alignment with:
    1. Baseline (mamba-65m-pretrained) 
    2. Llama-3.2-1B (alternative draft model)
    3. Advanced-mamba-alignment (previous training)
    Show if the improved training approach is better.
    """
    ...
    print(f"\n{'='*120}")
    print(f"IMPROVED MAMBA VS BASELINE, LLAMA-1B & ADVANCED TRAINING COMPARISON")
    print(f"Target: {target_name} | {tf32_name}")
    print(f"{'='*120}")
    ...
    # Separate improved vs advanced vs baseline vs llama
    improved_exps = [e for e in spec_exps if 'improved' in e['draft_model'].lower()]
    ...
    advanced_exps = [e for e in baseline_spec_exps if 'distilled' in e['draft_model'].lower()]
    
    if not improved_exps:
        print("Missing improved Mamba data for comparison!")
        return
    ...
    print(f"\n1. BASELINE (Pretrained) VS IMPROVED TRAINING")
    print(f"{'-'*120}")
    ...
    if llama1b_exps:
        print(f"\n2. LLAMA-3.2-1B VS IMPROVED MAMBA COMPARISON")
        print(f"{'-'*120}")
        print(f"Compare improved Mamba training with Llama-1B (different architecture)")
        print(f"{'-'*120}")
    ...
    # Now advanced comparison if available
    if not advanced_exps:
        print(f"\n3. Advanced training comparison: No advanced-mamba data found in baseline results")
        return
    
    # Only compare at step 1000 and best model where both trainings have checkpoints
    print(f"\n3. IMPROVED VS ADVANCED (DISTILLED) TRAINING COMPARISON")
    print(f"{'-'*120}")
    print(f"Note: Advanced training used different approach (distillation from Llama-2-7B)")
    print(f"Direct comparison only available at checkpoint-1000 and best_model")
    print(f"{'-'*120}")
    
    for sample_type in ['short', 'combined', 'long']:
        print(f"\n{sample_type.upper()} Prompts:")
        print(f"  {'Model':<20} | {'Improved (best K)':<25} | {'Advanced (best K)':<25} | {'Difference':<15}")
        print(f"  {'-'*20}+{'-'*27}+{'-'*27}+{'-'*17}")
        
        # Compare checkpoint-1000
        imp_1000 = [e for e in improved_exps 
                   if 'improved-1000' in e['draft_model'] and e['sample_set'] == sample_type]
        adv_1000 = [e for e in advanced_exps 
                   if 'distilled-checkpoint-1000' in e['draft_model'] and e['sample_set'] == sample_type]
        
        if imp_1000 and adv_1000:
            imp_best = max(imp_1000, key=lambda x: x['speedup_vs_auto'])
            adv_best = max(adv_1000, key=lambda x: x['speedup_vs_auto'])
            diff = imp_best['speedup_vs_auto'] - adv_best['speedup_vs_auto']
            diff_pct = (diff / adv_best['speedup_vs_auto']) * 100
            
            imp_str = f"{imp_best['speedup_vs_auto']:.3f}x (K={imp_best['lookahead']})"
            adv_str = f"{adv_best['speedup_vs_auto']:.3f}x (K={adv_best['lookahead']})"
            diff_str = f"{diff:+.3f}x ({diff_pct:+.1f}%)"
            
            print(f"  {'checkpoint-1000':<20} | {imp_str:<25} | {adv_str:<25} | {diff_str:<15}")
        
        # Compare best models
        imp_best_exps = [e for e in improved_exps 
                        if 'best' in e['draft_model'].lower() and e['sample_set'] == sample_type]
        adv_best_exps = [e for e in advanced_exps 
                        if 'best' in e['draft_model'].lower() and e['sample_set'] == sample_type]
        
        if imp_best_exps and adv_best_exps:
            imp_best = max(imp_best_exps, key=lambda x: x['speedup_vs_auto'])
            adv_best = max(adv_best_exps, key=lambda x: x['speedup_vs_auto'])
            diff = imp_best['speedup_vs_auto'] - adv_best['speedup_vs_auto']
            diff_pct = (diff / adv_best['speedup_vs_auto']) * 100
            ...
            print(f"  {'best_model':<20} | {imp_str:<25} | {adv_str:<25} | {diff_str:<15}")


def create_improved_mamba_plots(results, baseline_results, output_dir, target_filter=None, tf32_filter=None):
    """
    Create visualizations for improved Mamba training progression with baseline and Llama-1B comparison.
    """
    ...
    print(f"\nGenerating improved Mamba training plots for {target_filter}, TF32 {'ON' if tf32_filter else 'OFF'}...")
    ...
    if not improved_exps:
        print("No improved Mamba experiments found for plotting!")
        return
    
    # Prepare data
    fig, axes = plt.subplots(2, 3, figsize=(20, 12))
    fig.suptitle(f'Improved Mamba vs Llama-1B Training Progression\n{target_filter} | TF32 {"ON" if tf32_filter else "OFF"}', 
    ...
    # Plot 6: Mamba vs Llama-1B comparison by prompt type
    ax6 = axes[1, 2]
    ...
    bars1 = ax6.bar(x - width, baseline_vals, width, label='Baseline Mamba', color='lightcoral', alpha=0.8)
    bars2 = ax6.bar(x, improved_vals, width, label='Best Improved Mamba', color='forestgreen', alpha=0.8)
    bars3 = ax6.bar(x + width, llama_vals, width, label='Llama-3.2-1B', color='steelblue', alpha=0.8)
    ...
    ax6.axhline(y=1.0, color='black', linestyle='--', alpha=0.3, linewidth=1)
    ax6.set_xlabel('Prompt Type', fontsize=11)
    ax6.set_ylabel('Speedup', fontsize=11)
    ax6.set_title('Mamba vs Llama-1B Comparison', fontsize=12, fontweight='bold')
    ax6.set_xticks(x)
    ax6.set_xticklabels(sample_types)
    ...
    # Save figure
    output_path = output_dir / 'improved_mamba_training_analysis.png'
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python improved_mamba_analysis.py <sweep_results.json>")
        sys.exit(1)
    
    json_path = sys.argv[1]
    results = load_results(json_path)
    
    base_output = Path("outputs/improved_mamba_analysis")
    ...
    for target in targets:
        print(f"\n{'#'*120}")
        print(f"# IMPROVED MAMBA ANALYSIS: {target} | TF32 OFF")
        print(f"{'#'*120}")
    ...
    print(f"\n{'='*120}")
    print(f"✅ Improved Mamba training analysis complete!")
    print(f"{'='*120}")
