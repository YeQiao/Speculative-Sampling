...
class GuidedMamba2Mixer(nn.Module):
    ...
    def forward(
        ...
        """
        # Build kwargs compatible with both old (cache_position) and new transformers
        fwd_kwargs = dict(cache_params=cache_params, attention_mask=attention_mask)
        if cache_position is not None:
            fwd_kwargs["cache_position"] = cache_position

        if guidance_deltas is None:
            return self.m(hidden_states, **fwd_kwargs)
        ...
        try:
            out = self.m(hidden_states, **fwd_kwargs)
        finally:
